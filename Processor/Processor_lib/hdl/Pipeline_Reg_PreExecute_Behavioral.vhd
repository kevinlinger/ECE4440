...
ENTITY Pipeline_Reg_PreExecute IS
  Port( ALU0, ALU1, Extra : IN std_logic_vector (15 DOWNTO 0);
    ...
    out_ALU0, out_ALU1, out_Extra : out std_logic_vector (15 DOWNTO 0);
...
ARCHITECTURE Behavioral OF Pipeline_Reg_PreExecute IS
  ...
  BEGIN
    ...
    determine_bubble : process (Op_Type, bubble, clock, Kill, enable) is
    BEGIN
      if(Kill = '1') then
        Temp_Op_Type <= "000000001"; --nop
        elsif(enable = '1' and bubble = '1') then
          Temp_Op_Type <= "000000001"; --nop
          else
     ...
     determine_kill : process ( clock, Kill) is
    BEGIN
      if(Kill = '1') then
        Temp_enable <= '1'; --SMASH
      else
